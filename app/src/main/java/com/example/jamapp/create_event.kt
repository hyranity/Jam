...
class create_event : AppCompatActivity() {
    ...
    public fun createEvent(view : View){
        ...
        if (createEventDescription.text.length > 150) {
            val toast = Toast.makeText(
                applicationContext,
                "Event description should not exceed 150 characters.",
                Toast.LENGTH_SHORT
            )
            toast.show()
            return
        ...
        var newEvent = Event(
            event_id = event_id,
            host_id = auth.uid.toString(),
            title = createEventTitle.text.toString(),
            address = createEventVenue.text.toString(),
            description = createEventDescription.text.toString(),
